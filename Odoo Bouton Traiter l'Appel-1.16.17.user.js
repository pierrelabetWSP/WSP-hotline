...
(function() {
    ...
    // Fonction principale qui contient toute la logique du script
    function mainScript() {
        // Autoriser uniquement si l'URL contient "helpdesk.ticket&view_type"
        if (!window.location.href.includes("helpdesk.ticket&view_type")) {
            console.log("Script désactivé : pas une page helpdesk.ticket&view_type");
            return;
        }
            ...
            if (window.location.href.includes('model=helpdesk.ticket&view_type=form') && nomPharmacie) {
                ...
                const interval = setInterval(() => {
                    ...
                    if (champClient) {
                        ...
                        champClient.dispatchEvent(new Event('input', {
                            bubbles: true
                        }));
    ...
    `;
        ...
        observerBtnInitiales.observe(document.body, {
            childList: true,
            subtree: true
        });
   ...
   // ================== VERIFICATION URL ==================
    function verifierUrl() {
        ...
        if (url.includes("model=helpdesk.ticket&view_type=list")) {
            return "list";
        }
        if (url.includes("model=helpdesk.ticket&view_type=form")) {
            return "form";
        }

        console.log("🚫 Script désactivé : URL non pertinente ->", url);
        return null;
    }
